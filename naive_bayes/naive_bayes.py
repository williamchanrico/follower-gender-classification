...
import nltk
import numpy as np
from nltk import NaiveBayesClassifier, WordNetLemmatizer, classify, word_tokenize
...
def read_file(filename, data):
    ...
    for _, comment in enumerate(file["comments"]):
        words_in_comment = comment["text"].lower().translate(str.maketrans("", "", string.punctuation))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-l", "--limit", action="store", dest="limit", default=-1, type=int, help="Limit processed data")

    parser.add_argument("-m", "--model", action="store", dest="model", default="", type=str, help="Specify model file (pickle format)")
    ...
    parser.add_argument("-c", "--cache", action="store_true", dest="cache_model", default=False, help="Cache trained mode data")
